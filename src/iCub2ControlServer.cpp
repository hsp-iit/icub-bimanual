...
class iCub2ControlServer : public ControlInterface
{
	public:
		
		iCub2ControlServer();                                                               // Constructor
		
		//////////////// These are taken directly from ControlInterface.h //////////////////
		
		// Grasp an object given the 2 hand transforms
		bool grasp_object(const std::vector<double>& pose)
		{
			if(pose.size() == 12)
			{
		...
		bool move_object_to_pose(const std::vector<double>& pose)
		{
			if(pose.size == 6)
			{
				std::cout << "Worker bees can leave.\n";
		...
		bool move_hands_to_pose(const std::vector<double>& pose)
		{
			if(pose.size() == 12)
			{
				std::cout << "Worker bees can leave.\n";
		...
		bool move_joints_to_named_position(const std::string& positionName)
		{
			...
			{
				std::err << "[ERROR] [iCUB2 CONTROL SERVER] move_joints_to_named_position(): "
		...
		bool move_joints_to_position(const std::vector<double>& position)
		{
			return this->robot->move_to_position(EigenVectorXd(position.data()));
		}
