...
{
	this->_globalPose = globalToLocal*this->_localPose;                                         // Compute the transform of the object in global reference frame
	
	Eigen::Vector3d r = _globalPose.translation() - globalToLocal.translation();                // Translation from contact point to center
	Eigen::Vector3d w = contactTwist.tail(3);
	
	this->_twist.head(3) = contactTwist.head(3) + w.cross(r);                                   // v' = v + wxr  linear velocity
	this->_twist.tail(3) = contactTwist.tail(3);                                                // w' = w'       angular velocity
}
