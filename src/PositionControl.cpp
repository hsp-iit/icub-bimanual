	...
	: iCubBase(pathToURDF, jointList, portList, robotModel)
{
	...
	this->refToWalkingModule.open("/bimanualUpperRefs");
...
void PositionControl::run()
{
	if (update_state(this->qRef))
	{
		...
		dq.setZero(); // We want to solve for this
		...
		if (this->controlSpace == joint)
		{
			...
			if (this->_robotModel == "iCub2")
			{
				...
				if (QPSolver::last_solution_exists())
				{
					...
					for (int i = 0; i < this->numJoints; i++)
					{
						if (startPoint(i) <= lowerBound(i))
							startPoint(i) = lowerBound(i) + 0.1; // Just above the lower bound
						else if (startPoint(i) >= upperBound(i))
							startPoint(i) = upperBound(i) - 0.1; // Just below the upper bound
					}
				}
				else
			...
			else // this->robotModel == "ergoCub"
			{
				...
				for (int i = 0; i < this->numJoints; i++)
				{
					...
					if (dq(i) <= lowerBound(i))
						dq(i) = lowerBound(i) + 0.01; // Just above the lower bound
					else if (dq(i) >= upperBound(i))
						dq(i) = upperBound(i) - 0.01; // Just below the upper bound
				}
			}
		}
		else // this->controlSpace == Cartesian
		{
			...
			else // this->_robotModel = "ergoCub"
			{
				...
				if (QPSolver::last_solution_exists())
				{
					...
					{
						if (startPoint(i) <= lowerBound(i))
							startPoint(i) = lowerBound(i) + 0.01;
						else if (startPoint(i) >= upperBound(i))
							startPoint(i) = upperBound(i) - 0.01;
					}
				}
				else
					startPoint = 0.5 * (lowerBound + upperBound);
		...
		// if (not send_joint_commands(qRef))
		//	std::cout << "[ERROR] [POSITION CONTROL] Could not send joint commands for some reason.\n";
		...
		WalkingControllers::YarpUtilities::HumanState &ref_to_walking_val = this->refToWalkingModule.prepare();
		ref_to_walking_val.jointNames.clear();
		ref_to_walking_val.positions.clear();
		actualVector.clear();
		...
		for (int i = 0; i < this->numJoints; i++)
		{
			ref_to_walking_val.jointNames.push_back(this->jointList[i]);
			ref_to_walking_val.positions.push_back(this->qRef(i));
			actualVector.push_back(this->q(i) * 180.0 / M_PI);
			referenceVector.push_back(this->qRef(i));
			errorVector.push_back(180.0 / M_PI * (this->qRef(i) - this->q(i)));
		}
		this->refToWalkingModule.write();
...
Eigen::Matrix<double, 12, 1> PositionControl::track_cartesian_trajectory(const double &time)
{
	...
	dx.setZero();						  // Value to be returned
...
{
	Eigen::VectorXd dq(this->numJoints);
	dq.setZero(); // Value to be returned
	...
	if (mu > this->threshold) // i.e. not singular
	{
		...
		if (QPSolver::last_solution_exists())
		{
			...
			if (lastSolution.size() == (12 + this->numJoints))
				startPoint = lastSolution; // Lagrange multipliers & joint control
			else
			{
			...
			for (int i = 0; i < this->numJoints; i++)
			{
				if (startPoint(12 + i) <= lowerBound(i))
					startPoint(12 + i) = lowerBound(i) + 0.01;
				else if (startPoint(12 + i) >= upperBound(i))
					startPoint(12 + i) = upperBound(i) - 0.01;
			}
		}
		else
		{
	...
	if (this->isGrasping) // Solve again subject to grasp constraints
	{
		...
		try
		{
			dq = (QPSolver::solve(H, f, this->B.block(0, 6, 10 + 2 * this->numJoints, 6 + this->numJoints),
								  z, startPoint))
					 .tail(this->numJoints); // Remove Lagrange multipliers from the solution
		}
		catch (const std::exception &exception)
		{
			std::cout << exception.what() << std::endl;
