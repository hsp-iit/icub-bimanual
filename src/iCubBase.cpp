	  ...
	  desiredPosition(Eigen::VectorXd::Zero(this->numJoints))		 // Desired configuration when running Cartesian control
{
	...
	else
	{
		...
		{
			message += "Could not generate iDynTree::KinDynComputations object from the model " + loader.model().toString() + ".";
		...
		else
		{
			...
			if (not update_state())
				throw std::runtime_error(message + "Unable to read initial joint state from the encoders.");
								...
								const double &time)
{
	...
	else
	{
		...
		target.push_back(position); // Insert in to std::vector to pass onward
		std::vector<double> times;
		times.push_back(time);					 // Time in which to reach the target position
								 ...
								 const std::vector<double> &times)
{
	...
	{
		if (isRunning())
			stop();							 // Stop any control thread that might be running
		...
		for (int i = 0; i < this->numJoints; i++) // For the ith joint...
		{
			for (int j = 0; j < m; j++) // ... and jth waypoint
			{
				...
				else
				{
					...
					if (target < this->positionLimit[i][0])
						target = this->positionLimit[i][0] + 0.001;
					else if (target > this->positionLimit[i][1])
						target = this->positionLimit[i][1] - 0.001;
			...
			if (not this->jointTrajectory[i].setData(t, waypoint))
			{
				...
				this->jointTrajectory[i].setInitialConditions(this->qdot[i], 0.0); // Use the current joint velocity
		}
							 ...
							 const std::vector<double> &times)
{
	...
	std::vector<double> t;
	t.push_back(0.0);							   // Start immediately
	t.insert(t.end(), times.begin(), times.end()); // Add on the rest of the times

	// Set up the waypoints for each hand
	std::vector<Eigen::Isometry3d> leftPoints;
	leftPoints.push_back(this->leftPose); // First waypoint is current pose
	leftPoints.insert(leftPoints.end(), left.begin(), left.end());

	std::vector<Eigen::Isometry3d> rightPoints;
	rightPoints.push_back(this->rightPose);
...
bool iCubBase::grasp_object()
{
	...
	else
	{
		...
		if (update_state())
			return move_object(this->payload.pose(), 1.0); // Update pose, grasp constraints, activate control
		else
		{
						   ...
						   const std::vector<double> &times)
{
	...
	// Set up the times for the trajectory
	std::vector<double> t;
	t.push_back(0);								   // Start immediately
	t.insert(t.end(), times.begin(), times.end()); // Add on the rest of the times

	// Set up the waypoints for the object
	std::vector<Eigen::Isometry3d> waypoints;
	waypoints.push_back(this->payload.pose());					   // First waypoint is current pose
...
Eigen::Isometry3d iCubBase::hand_pose(const std::string &which)
{
	if (which == "left")
		...
		throw std::invalid_argument("[ERROR] [iCUB BASE] hand_pose(): Expected 'left' or 'right' but the argument was '" + which + "'.");
}
...
Eigen::MatrixXd iCubBase::partial_derivative(const Eigen::MatrixXd &J, const unsigned int &jointNum)
{
	...
	Eigen::MatrixXd dJ(6, this->numJoints);
	dJ.setZero();
...
{
	Eigen::VectorXd task(this->numJoints);
	task.setZero(); // Value to be returned
	...
	default: // manipulability
	{
		...
		for (int i = this->numJoints - 1; i >= 0; i--)
		{
			...
			{
				task(i) = this->kr * this->manipulability * (JJt_right * partial_derivative(this->Jright, i) * this->Jright.transpose()).trace();
			}
