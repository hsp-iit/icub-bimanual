...
RUN apt update && apt install --no-install-recommends -y -qq libgazebo11-dev
...
RUN git config --global user.name "user" && \
    ...
    cmake -DROBOTOLOGY_ENABLE_CORE=ON -DROBOTOLOGY_USES_GAZEBO=ON -DROBOTOLOGY_USES_PYTHON=ON -DROBOTOLOGY_USES_LUA=ON -DYCM_EP_ADDITIONAL_CMAKE_ARGS:STRING="-DICUB_COMPILE_BINDINGS:BOOL=ON -DCREATE_PYTHON:BOOL=ON -DENABLE_yarpmod_RGBDSensorWrapper:BOOL=ON -DENABLE_yarpmod_RGBDSensorClient:BOOL=ON -DENABLE_yarpcar_mjpeg:BOOL=ON -DENABLE_yarppm_depthimage_to_rgb:BOOL=ON -DENABLE_yarppm_depthimage_compression_zlib:BOOL=ON" ../ && \
    make -j1
#    echo "source /robotology-superbuild/build/install/share/robotology-superbuild/setup.sh" >> /home/ecub/.bashrc
RUN echo "source /robotology-superbuild/build/install/share/robotology-superbuild/setup.sh" >> /root/.bashrc
